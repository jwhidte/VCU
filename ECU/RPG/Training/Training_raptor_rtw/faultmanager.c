...
void setOccurredState(int16_T faultIdx, boolean_T nextValue)
{
  ...
  if (occurred_changed) {
...

    {
      uint16_T i;
      for (i = 0; i < 2; i++) {
        E_FaultState famState = faultActionMapState_Data()[faultIdx-1][i];
        int16_T famAction = faultActionMapAction_Data()[faultIdx-1][i];
        if ((famAction >= 0) && (famAction < 1)) {
          if ((famState != 0) && ((famState & FAULT_OCCURRED_MASK) != 0)) {
            if (nextValue)
              faultActionCount_Data()[famAction]++;
            else if (faultActionCount_Data()[famAction] != 0)
              faultActionCount_Data()[famAction]--;
          }
        }
      }
  ...
  if (notoccurred_changed) {
...

    {
      ...
      for (i = 0; i < 2; i++) {
        ...
        int16_T famAction = faultActionMapAction_Data()[faultIdx-1][i];
        if ((famAction >= 0) && (famAction < 1)) {
          if ((famState != 0) && ((famState & FAULT_NOTOCCURRED_MASK) != 0)) {
            if (!nextValue)
              faultActionCount_Data()[famAction]++;
            else if (faultActionCount_Data()[famAction] != 0)
              faultActionCount_Data()[famAction]--;
  ...
  if (curr_value != nextValue) {
...

    {
      ...
      for (i = 0; i < 2; i++) {
        ...
        int16_T famAction = faultActionMapAction_Data()[faultIdx-1][i];
        if ((famAction >= 0) && (famAction < 1) &&
            (famState != 0) && ((famState & FAULT_ACTIVE_MASK) != 0)) {
          if (nextValue)
            faultActionCount_Data()[famAction]++;
          else if (faultActionCount_Data()[famAction] != 0)
            faultActionCount_Data()[famAction]--;
  ...
  if (curr_value != nextValue) {
...

    {
      ...
      for (i = 0; i < 2; i++) {
        ...
        int16_T famAction = faultActionMapAction_Data()[faultIdx-1][i];
        if ((famAction >= 0) && (famAction < 1) &&
            (famState != 0) && ((famState & FAULT_SUSPECTED_MASK) != 0)) {
          if (nextValue)
            faultActionCount_Data()[famAction]++;
          else if (faultActionCount_Data()[famAction] != 0)
            faultActionCount_Data()[famAction]--;
...
{
  int16_T faultIdx;
  for (faultIdx = 1; faultIdx <= 4; faultIdx++) {
    clearFault(faultIdx);
  }
}
...
int16_T getNextFaultByAction(int16_T action, int16_T startIdx)
{
  ...
  if ((action < 1) && (action >= 0)) {
    ...
    if (nextIdx < 0)
      nextIdx = 0;
    nextIdx++;
    for (faultIdx = nextIdx; faultIdx <= 4; faultIdx++) {
      for (actIdx = 0; actIdx < 2; actIdx++) {
        E_FaultState famState = faultActionMapState_Data()[faultIdx-1][actIdx];
        int16_T famAction = faultActionMapAction_Data()[faultIdx-1][actIdx];
        if (getFaultState(faultIdx, famState) && (famAction == action))
          return faultIdx;
        else if (famState == 0)
          break;
      }
...
boolean_T getFaultActionState(int16_T action)
{
  if ((action >= 0) && (action < 1))
    return faultActionCount_Data()[action] > 0;
  return 0;
}
